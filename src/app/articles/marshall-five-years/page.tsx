  ...
  return (
    <FadeIn className="mf-section">
      <div className="mf-body-prose">
        ...
        <p>
          The story the record tells is not the story that was predicted. Insurance &mdash; the fight everyone expected &mdash; left the council chamber within a year. What replaced it was a slow, four-year rezoning around the damage: ADUs, short-term rentals, grassland fuel management, and the adjacency of rebuilt lots to newly approved density. <a href="https://www.myhamlet.com/search?q=marshall+fire&ref=district">Residents, trustees, and planning commissioners</a> were not talking about carriers. They were talking about what gets built next to them, and who will own it.
        </p>
      </div>
    </FadeIn>
  );
...
function InsuranceSilenceSection() {
  ...
  return (
    <section id="the-silence" className="mf-section mf-section-border">
      ...
      <FadeIn className="mf-body-prose">
        <p>
          In the first year after the fire, insurance came up in {years[0].chunks} chunks of burn-zone council testimony. In 2023 that fell to {years[1].chunks}. In 2024, {years[2].chunks}. In 2025, {years[3].chunks}. In the 2026 record through mid-April, {years[4].chunks}. Across {DATA.meetings.totalTranscribed.toLocaleString()} meetings, insurance generated {DATA.quoteTopics.find(q => q.topic === "Insurance")?.count} extracted quotes &mdash; a small share of the quote pool but with the highest per-quote intensity of any topic: {DATA.quoteTopics.find(q => q.topic === "Insurance")?.avgIntensity} out of 5. When residents spoke about insurance, they spoke at the ceiling of testimony intensity. Then they stopped speaking about it at this level of government.
        </p>
      </FadeIn>
...
function RoutinizationSection() {
  ...
  return (
    <section id="the-routinization" className="mf-section mf-section-border">
      ...
      <FadeIn className="mf-body-prose">
        <p>
          Four rhetorical dimensions were scored on every transcript chunk: urgency, contentiousness, personal testimony, and technical complexity. Each is a 1&ndash;5 relative measure &mdash; not a calibrated scientific index, but a consistent scoring of language patterns.
        </p>
        <p>
  ...
  return (
    <section id="voices" className="mf-section mf-wide-section">
      <div className="mf-section-header">
        ...
        <p className="mf-section-sub">Eleven featured testimonies pulled from the four-year post-Marshall council record, curated by Gemini 3 Pro from a filtered pool of {DATA.meetings.filteredQuotePool.toLocaleString()} candidates. Each card links to the exact archived meeting.</p>
      </div>
